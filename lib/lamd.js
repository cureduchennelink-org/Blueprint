...
//   CG had an out-of-memory issue and got 'topology destroyed'; see 3rd answer (and later answer for mongo directly) https://stackoverflow.com/questions/30909492/mongoerror-topology-was-destroyed
...
class Lamd {
	static deps() {
		return {services:[ 'logger'], config: 'lamd'};
	}
	constructor(kit){
	...
	async server_init_promise(kit, promise_chain){ // Return the promise_chain
		...
		const res = await MongoClient.connect(this.config.connect_url);
		console.log(f, { res })
