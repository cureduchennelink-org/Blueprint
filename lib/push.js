...
(function() {
  ...
  Push = (function() {
    ...
    Push.prototype.GetPushSet = function(clear_pset, nm) {
      ...
      return Promise.resolve().bind(this).then(function() {
        ...
        this.pset_by_name[nm] = new PushSet(this.sdb, pset_rec, this.util);
        pset_id = pset_rec.id;
        if (!clear_pset) {
          return false;
        }
  ...
  PushSet = (function() {
    function PushSet(sdb, pset1, util) {
      this.sdb = sdb;
      this.pset = pset1;
      this.util = util;
      this.c_items = {};
    ...
    PushSet.prototype.GetPushHandle = function(ctx, xref) {
      ...
      return Promise.resolve().bind(this).then(function() {
        ...
        item = item_rec;
        return this.sdb.pset_item_change.GetMostRecentForItem(ctx, this.pset.id, item_rec.id);
      }).then(function(db_rows) {
        if (!db_rows.length) {
          throw new this.E.ServerError("PUSHSET:GET_HANDLE:NO_LATEST_CHANGE", this.pset.id + "/" + item_rec.id + "/?");
        }
