...
(function() {
  ...
  Kit = (function() {
    ...
    Kit.prototype.get_service_deps_needed = function(name, constructor) {
      ...
      d = constructor.deps;
      if (!d) {
        throw new Error(("Module '" + name + "' is missing @deps - ") + JSON.stringify(constructor));
      }
      needed = [];
      ref = d.services || [];
      results = [];
    ...
    Kit.prototype.new_service = function(name, constructor, args) {
      ...
      if (!d) {
        throw new Error("Service-Module '" + name + "' is missing @deps");
      }
      ...
      for (i = 0, len = needs.length; i < len; i++) {
        ...
        if (!(n in this.services)) {
          throw new Error("Service-Module '" + name + "' requires service '" + n + "'");
        }
      }
      _t = this;
      _a = args != null ? args : [];
    ...
    Kit.prototype.new_route_service = function(name, constructor, args) {
      ...
      if (!d) {
        throw new Error("Route-Module '" + name + "' is missing @deps");
      }
      needs = (ref = d.services) != null ? ref : [];
      needs.push('wrapper');
      for (i = 0, len = needs.length; i < len; i++) {
        n = needs[i];
        if (!(n in this.services)) {
          throw new Error("Route-Module '" + name + "' requires service '" + n + "'");
        }
      }
      _t = this;
      _a = args != null ? args : [];
