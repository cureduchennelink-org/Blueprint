...
class Auth {
	...
	_pbkdf2(p,buf,IT,KL){ return crypto.p_pbkdf2(p, buf, IT, KL, 'sha1'); }
	...
	server_use(req, res, next){
		...
		req.auth= {
			...
			token: result.token,
			authId: result.token ? result.token.id : null,
			tenant: result.token ? result.token.tenant : null,
			role: (result.token && result.token.role!= null
				? Array.isArray(result.token.role)
					? result.token.role
					: result.token.role.split(',')
				: null),
			authorize: async (ctx, skip_response)=> {
				if (!req.auth.authId) {
					if (skip_response) { return false; }
					console.log('Auth:server_use.authorize;', req.auth)
					const error= new this.E.OAuthError(401, 'invalid_token', req.auth.message);
	...
	ValidateCredentials(ctx, username, password){
		...
		.then(function() {
			...
			return this.sdb.auth.GetAuthCreds(ctx, username);}).then(function(db_rows){
		...
		.then(function(a_match){
			if (!a_match) { throw new this.E.OAuthError(401, 'invalid_client'); }
	...
	ComparePassword(password, compareHash){
		...
		if (parts.length !== 2) { throw new this.E.ServerError('auth_error','Missing salt on password hash'); }

		return (this._pbkdf2(password, new Buffer(parts[0], 'base64'), this.ITERATIONS, this.KEY_LENGTH))
		.then(function(key){
	...
	EncryptPassword(password){
		...
		.then(function() {
			...
			// Create Salt
			return crypto.p_randomBytes(this.SALT_SIZE);}).then(function(buffer){
			saltBuf= buffer;

			// Encrypt Password
			return this._pbkdf2(password, saltBuf, this.ITERATIONS, this.KEY_LENGTH);}).then(key => (saltBuf.toString('base64')) + '.' + new Buffer(key).toString('base64'));
	}
}
