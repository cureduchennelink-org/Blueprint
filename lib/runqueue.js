...
class RunQueue {
	...
	constructor(kit) {
		...
		const f= 'RunQueue::constructor';
		this.log= 		kit.services.logger.log;
		this.E= 		kit.services.error;
		this.sdb= 		kit.services.db.mysql; // TODO: Let's make this a conditional based on what we need. Mongo / MySQL
		this.config=	kit.services.config.runqueue;

		this.ctx_poll= {log: this.log, conn: false}; // Poller will populate conn-ection on demand
		this.poll_timer_id= false;
		this.poll_processing= false; // To keep from poller running on top of us
		this.job_cnt= 0; // Active jobs running on this server; Should not exceed @config.settings.jobs

		this.ctx_finish= {log: this.log, conn: false}; // Poller will populate on demand
	...
	server_start(kit){ // After the services are all created, we need to validate/load our dynamic references per topic
		...
		const interval_ms= this.config.settings.poll_interval_ms;
		if (interval_ms !== false) { this.poll_timer_id= setInterval(this._Poll, interval_ms); } // False for unit-testing
	...
	Drain() {  // Stop taking in new job requests, the server is coming down
		const f= 'RunQueue::Drain:';
		this.log.info(f, {was_draining: this.poll_timer_id === false});
	...
	AddJob(ctx, details, job_id){ // Caller provides promise wrapper (internal, job_id set if 'replace'
		...
		const allowed_details= _.pick(details, [ 'topic', 'unique_key', 'priority', 'json', ]);
	...
	_Poll() {
		let rec;
		const f= 'RunQueue::_Poll:';
		const rVal= []; // Returned to caller for testing (when unit tests call us directly)
		this.log.debug(f, {poll_processing: this.poll_processing, job_cnt: this.job_cnt});
		if (this.poll_processing === true) { return {stop: 'poll_processing'}; }
		...
		return Promise.resolve().bind(this)
		.then(function() {
			if (ctx.conn !== false) { return false; }
			...
			return this.sdb.runqueue.GetPendingCnts(ctx);}).then(function(db_rows){
			for (rec of Array.from(db_rows)) { group_cnt[ rec.group_ref]-= rec.active_cnt; }
			rVal.push({post_group_cnt: _.clone(group_cnt)});

			return this.sdb.runqueue.GetNextJobs(ctx, this.config.settings.read_depth);}).then(function(db_rows){
			rVal.push({next_jobs: db_rows});
			if (db_rows.length === 0) { // No work in this block
				this.poll_processing= false;
				return false;
			}

			let q_result= Promise.resolve().bind(this);
			for (job of Array.from(db_rows)) {
				if (group_cnt[ job.group_ref]< 1) { continue; }
				group_cnt[ job.group_ref]--; // Someone will run this, or we will
				(job=> {
					// We have to fight for the right to run this job
					return q_result= q_result // Serialize behind one promise for this poll interval
					.then(function() {
						let reread;
						...
						return this.sdb.runqueue.MarkJobPending(ctx, job.id, {fail_at}, (reread= true));}).then(function(db_rows){
						rVal.push({MarkJobPending_result: db_rows});
						if (db_rows.length !== 1) { return [false]; }

						// Wrap in a promise and catch error as a topic_result w/failure setting
						return Promise.resolve().bind(this)
						.then(function() {
							return this.topics[ job.topic]._method(job);}).then(function(topic_result){
					...
					.then(function(...args){
						let topic_result;
						let job;
						[job, topic_result] = Array.from(args[0]);
						if (job === false) { return false; } // Some other server ran it first

						return this._ProcessTopicResult(job, topic_result);}).then(process_result => rVal.push({process_result}));
				})(job);
			}

			this.poll_processing= false; // Allow a new poll request, even though the promise-chain of 'attempt to mark-n-lauch jobs' is only just established
			return q_result;}).then(() => rVal);
	}
	...
	_ProcessTopicResult(job, topic_result){
		const f= 'RunQueue::_ProcessTopicResult:';
		const ctx= this.ctx_finish;
		...
		return this.finish_promise= this.finish_promise
		.then(function() {
			return Promise.resolve().bind(this)
			.then(function() {
				if (topic_result.success === true) { // Else we consider it a failure
					...
					return this.sdb.runqueue.Fail(ctx, job.id, {run_at, last_reason: topic_result.reason}, (reread= true));
				}}).catch(function(err){
...
RunQueue.initClass(); // TODO CONSIDER WHAT IMPACT RETURNING AN ERROR HERE HAS ON THE REST OF THE PROMISE CHAIN
