...
(function() {
  var Promise, RunQueue, VALID_UNITS, _, goo_runqueue, it_is, moment,
  ...
  goo_runqueue = require('./goo_runqueue');
  ...
  RunQueue = (function() {
    ...
    RunQueue.prototype.server_start = function(kit) {
      ...
      return Promise.resolve().bind(this).then(function() {
        return this.sdb.runqueue.open(this.config.mongodb_uri, this.config.mongodb_name);
      }).then(function(c) {
    ...
    RunQueue.prototype._pick_at = function(retries, which, topic, other_object) {
      ...
      if (it_is.array(at)) {
        ...
        return moment(at).add(this._back_off_strategies[back_off_strategy](retries), 's');
      }
    };
    ...
    RunQueue.prototype.RemoveJobsByUniqueIds = function(uniqueIds) {
      ...
      this.finish_promise = this.finish_promise.then(function() {
        return Promise.resolve().bind(this).then(function() {
          return this.sdb.runqueue.RemoveByUniqueKeys(this.ctx_finish, uniqueIds);
        })["catch"](function(e) {
          return this.log.error(f, e);
        });
      });
    ...
    RunQueue.prototype._set_status = function(status_obj, status, detail_name, row) {
      ...
      if ((base1 = status_obj.details)[detail_name] == null) {
        base1[detail_name] = [];
      }
    ...
    RunQueue.prototype.HealthCheck = function(ctx) {
      ...
      if (this.poll_timer_id === false) {
        this._set_status(status, y, 'draining', true);
      }
      return Promise.resolve().bind(this).then(function() {
    ...
    RunQueue.prototype._ProcessTopicResult = function(job, topic_result) {
      ...
      if ('error' in topic_result) {
        topic_result = {
          success: false,
          reason: topic_result.error.toString()
        };
