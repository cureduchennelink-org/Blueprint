...
class RunQueue {
	...
	constructor(kit) {
		...
		console.log('RUNQUEUE',  this.config); // XXX
	...
	server_start(kit){ // After the services are all created, we need to validate/load our dynamic references per topic
		...
		// Don't retun the promise; should not complete ever
	...
	AddJob(ctx, details, job_id){ // Caller provides promise wrapper (internal, job_id set if 'replace'
		...
		// Pull values that will become DB colunm names
	...
	async _PollWrapper() {
		...
		console.log( 'RUNQUEUE', {interval}); // XXX

		// XXX FIX DRAIN: this.pollerInterval = setInterval(async () => {
		while (true){

			console.log( 'RUNQUEUE:BEFORE', {interval}); // XXX
			await Promise.delay( interval);
			console.log( 'RUNQUEUE:AFTER:DELAY', {interval}); // XXX
			const pollerIsHung = await this._HandleHungPoller()
			console.log( 'RUNQUEUE:AFTER:HUNG', {pollerIsHung}); // XXX
			...
			console.log( 'RUNQUEUE:BEFORE:ACQUIRE', {ctx}); // XXX
			const freshConnection = await this.db.core.Acquire()
			console.log( 'RUNQUEUE:AFTER:ACQUIRE', {interval}); // XXX
			ctx.conn = freshConnection

			try {
				this.pollProcessing = true;
				console.log( 'RUNQUEUE:BEFORE:POLL', {pollProcessing: this.pollProcessing}); // XXX
				const poll_result= await this._Poll(ctx)
				console.log( 'RUNQUEUE:DURING:POLL', {poll_result}); // XXX
				this._UpdatePollDelay()
				this.pollProcessing = false;
				console.log( 'RUNQUEUE:AFTER:POLL', {pollProcessing: this.pollProcessing}); // XXX
			} catch(e) {
				console.log( 'RUNQUEUE:POLL:ERROR', e); // XXX
				await this.slack.send(e)
			} finally {
				console.log( 'RUNQUEUE:FINALLY', {interval}); // XXX
				this.db.core.release(ctx.conn);
				ctx.conn = false;
			}
			console.log( 'RUNQUEUE:BOTTOM', {interval}); // XXX
		}
		// XXX }, interval)
	}
	...
	async _Poll(ctx) {
		...
		this.log.debug(`${f} :>> POLLER HEARTBEAT.`) // XXX
		...
		this.log.debug(`${f} :>> ${nextJobs.length} JOB${nextJobs.length > 1 ? 'S' : ''} FOUND.`)
