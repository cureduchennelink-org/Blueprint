...
class RunQueue {
	...
	server_start(kit){ // After the services are all created, we need to validate/load our dynamic references per topic
		...
		if (interval_ms !== false) { this.poll_timer_id= setInterval(() => {
			this._Poll()
			this._updatePollDelay()
		}, interval_ms); } // False for unit-testing
	...
	async _Poll() {
		...
		const dbConnection = await this.sdb.core.Acquire()
		if (!dbConnection) throw new this.E.ServerError("UNABLE TO ACQUIRE DATABASE CONNECTION");
		...
		await Promise.map(nextJobs, async (job) => {
			...
			const processedResult = await this._ProcessTopicResult(job, topicResult);
