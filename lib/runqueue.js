...
//
var {_jsonReplacer}=require('./json');
...
class RunQueue {
	...
	constructor(kit) {
		...
		const databaseToUse = this.config.whatDatabaseToUse || "psql";
		this.sdb = kit.services.db[databaseToUse];
		this.db = kit.services.db[databaseToUse];

		this.globalConfig = kit.services.config;

		this.defaults = {
			interval: 5000,
		};

		this.delayLog = this.config.settings.delayLog || false;
	...
	server_start(kit) {
		...
		if (this.globalConfig.runq && !this.globalConfig.runq.on) return;
		for (let nm in this.topics) {
			...
			if (!(service in kit.services)) {
				throw new Error(
					`KIT DOES NOT HAVE SERVICE (${service}) MENTIONED IN TOPIC (${nm}) AS [${topic.service}]`
				);
			}
			...
			if (!it_is.function(this._back_off_strategies[topic.back_off])) {
				throw new Error(
					`UNKNOWN BACK OFF STRATEGY ${
						topic.back_off
					} IN TOPIC ${nm} should be one of [${Object.keys(
			...
			if (!(method in kit.services[service])) {
				throw new Error(
					`METHOD (${method}) NOT FOUND IN SERVICE (${service}) MENTIONED IN TOPIC (${nm}) AS [${topic.service}]`
				);
			}
			topic._method = kit.services[service][method];
			if (!it_is.function(topic._method)) {
				throw new Error(
					`NOT A FUNCTION: (${topic.service}) MENTIONED IN TOPIC (${nm})`
				);
			}
			topic.alarm_delay_sec = this._calc_secs(
				topic.alarm_delay,
				`${nm}.alarm_delay`
			);
			topic.warn_delay_sec = this._calc_secs(
				topic.warn_delay,
				`${nm}.warn_delay`
			);
		}
	...
	_pick_at(retries, which, topic, other_object) {
		// E.g. 0, 'run_at', topic_as_str_or_@topics[nm], users_object_with_optional_override
		let at;
		...
		if (it_is.array(at)) {
			return this._calc_at(
				this._back_off_strategies[back_off_strategy](retries),
				at,
				`${resolved_topic.nm}:${which}`
			);
		} else {
	...
	_validate_format(spec, name) {
		if (
			...
			it_is.number(spec[0]) &&
			it_is.string(spec[1]) &&
			Array.from(VALID_UNITS).includes(spec[1])
		) {
			return true;
		} else {
			throw new Error(
				`SPEC (${name}) WAS NOT AN ARRAY OF NUMBER AND STRING-UNIT (${spec})`
			);
		}
	}
	...
	AddJob(ctx, details, job_id) {
		// Caller provides promise wrapper (internal, job_id set if 'replace'
		if (job_id == null) {
			job_id = false;
		}
		...
		const defaults = _.pick(topic, ["priority", "unique_key"]); // User can override these

		const only_topics = _.pick(topic, ["group_ref"]);
		const allowed_details = _.pick(details, [
			"topic",
			"unique_key",
			"priority",
			"json",
		...
		return Promise.resolve()
			...
			.then(function () {
				...
				if (job_id === false) {
					...
					const replace_values = _.pick(new_values, [
						// List from sql layer for ReplaceJob
						"unique_key",
						"priority",
						"run_at",
						"json",
					]);
					return this.sdb.runqueue.ReplaceJob(
						ctx,
						job_id,
						replace_values,
						(reread = true)
			...
			.catch(function (e) {
				if (
					(e.errno === 1062 &&
						e.sqlMessage.includes("ix_runqueue__unique_key")) ||
					(e.code === 11000 &&
						e.errmsg.includes("duplicate key error collection"))
				) {
	...
	_UpdatePollDelay() {
		if (this.pollLogDelay !== false && !moment().isAfter(this.pollLogDelay))
			return;

		// TODO: Set defaults

		let quantity = "10";
		let measurement = "minutes";
		if (this.config.settings.pollLogDelay) {
			const pollLogDelay = this.config.settings.pollLogDelay;
			quantity = pollLogDelay.quantity;
			measurement = pollLogDelay.measurement;
		}
		this.pollLogDelay = moment().add(quantity, measurement);
	}

	_getInterval() {
		const interval = this.config.settings.poll_interval_ms;
	...
	_PollWrapper() {
		...
		if (interval === false) {
			this.log.debug(
				`${f}::INTERVAL SET TO FALSE. YOU ARE EXPLICITLY TURNING THE POLLER OFF!`
			);
			return;
		}
	...
	_logPoller() {
		...
		if (this.delayLog === false) {
			this.log.debug(`${f} :>> POLLER HEARTBEAT.`);
			return;
		}

		const logPolling =
			this.pollLogDelay === false || moment().isAfter(this.pollLogDelay);
	...
	async _Poll(ctx) {
		...
		for (rec of Array.from(pendingCounts)) {
			group_cnt[rec.group_ref] -= rec.active_cnt;
		}
		rVal.push({ post_group_cnt: _.clone(group_cnt) });

		const nextJobs = await this.sdb.runqueue.GetNextJobs(
			ctx,
			this.config.settings.read_depth
		);
		...
		this.log.debug(
			`${f} :>> ${nextJobs.length} JOB${nextJobs.length > 1 ? "S" : ""} FOUND.`
		);

		await Promise.map(nextJobs, async (job) => {
			if (group_cnt[job.group_ref] < 1) return;
			console.log("this.topics :>> ", this.topics);
			console.log("job.topic :>> ", job.topic);
			const fail_at = this._calc_at(
				0,
				this.topics[job.topic].fail_at,
				job.topic + ".fail_at"
			); // COULD THROW ERROR AND NOT PROCESS REST OF JOBS!
			const jobsPending = await this.sdb.runqueue.MarkJobPending(
				ctx,
				job.id,
				{ fail_at },
				true
			...
			const processedResult = await this._ProcessTopicResult(
				ctx,
				job,
				topicResult
			);
	...
	async _ProcessTopicResult(ctx, job, topic_result) {
		...
		if ("error" in topic_result) {
			// Caught error from running their logic
			topic_result = {
				success: false,
				reason: JSON.stringify(topic_result.error,_jsonReplacer(true))
			};
		} else if (!("success" in topic_result)) {
			topic_result = {
				success: false,
				reason: "Bad Response: " + JSON.stringify(topic_result,_jsonReplacer(true)),
			};
		} else if (('reason' in topic_result) && typeof topic_result.reason !== 'string') {
			topic_result.reason= JSON.stringify( topic_result.reason, _jsonReplacer( true));
		}

		this.log.debug(`${f}:TOPIC RESULT FOR ${job.topic} :>>`, { topic_result });
		...
		try {
			if (topic_result.success === true) {
				if (topic_result.replace) {
					...
					); // Allow topic override, but know sql won't change it
			...
			if (job.retries === 3) {
				...
				if (topicDetails) {
					...
					await this.slack.send(
						`${job.topic} IS FAILING. \n\n HERE'S WHY: ${topic_result.reason}`
					);
				}
			}

			const run_at = this._pick_at(
				job.retries + 1,
				"run_at",
				job.topic,
				topic_result
			);
			return this.sdb.runqueue.Fail(
				ctx,
				job.id,
				{ run_at, last_reason: topic_result.reason },
				true
			);
		} catch (err) {
