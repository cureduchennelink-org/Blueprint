...
class PushManager {
	...
	async _Poll() {
		...
		await _ctx_wrap(f, this, on_error, async (ctx) => {
			...
			db_rows = await this.sdb.push_item.GetNext({ ...ctx, silent: true }, this.last_item_id, limit);
		});
		if (db_rows.length) {
	...
	async MakeItem(ctx, route, slice, payload) {
		...
		const route_slice = _route_slice(route, slice);
		let db_results, new_values;

		// Insert the change
		new_values = { route_slice, payload, };
		db_results = await this.sdb.push_item.create(ctx, new_values);
		if (db_results.affectedRows !== 1) throw new this.E.DbError(f + 'create:' + db_results.affectedRows);
		return true;
	}
