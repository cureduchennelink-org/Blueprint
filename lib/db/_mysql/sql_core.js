	...
	constructor(kit, pool_opts){
		...
		this.sqlQuery= (ctx, sql, args)=> {
			...
			.then(() => query(sql, args)).then(function(just_rows){
				...
				if ( !Array.isArray( just_rows)) { just_rows.time_ms= Date.now()- start_time; }
				if (this.is_db_log_on) { ctx.log.debug(`${f}result:`, (!Array.isArray(just_rows || (just_rows.length < 3)) ? just_rows : ({time_ms: Date.now()- start_time, rows: just_rows.length, trunc: just_rows.slice(0, 3)}))); }
