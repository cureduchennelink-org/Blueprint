  ...
  SqlCore.prototype.method_factory = function(sql_mod, name) {
    ...
    if (schema.get_collection) {
      sql_mod.get_collection = function(ctx) {
        ...
        return Q.resolve().then(function() {
          var sql;
          sql = 'SELECT ' + (schema.get_collection.join(',')) + ' FROM ' + table + ' WHERE di= 0';
    ...
    if (schema.create) {
      sql_mod.create = function(ctx, new_values, re_read) {
        var f, nm, result, val,
          _this = this;
        f = "DB:" + name + ":create:";
    ...
    if (schema.update_by_id) {
      sql_mod.update_by_id = function(ctx, id, new_values, re_read) {
        var f, nm, result, val,
          _this = this;
    ...
    if (schema.delete_by_id) {
      return sql_mod.delete_by_id = function(ctx, id) {
        var f,
          _this = this;
