  ...
  SqlAuth = (function() {
    ...
    SqlAuth.prototype.GetAuthCreds = function(ctx, cred_name) {
      ...
      return Promise.resolve().bind(this).then(function() {
        var sql;
        sql = 'SELECT ' + (this.schema.auth.join(',')) + ' FROM ' + this.table + ' WHERE ' + this.cred_col + '= ? and di= 0';
    ...
    SqlAuth.prototype.GetByCredName = function(ctx, cred_name) {
      ...
      return Promise.resolve().bind(this).then(function() {
        var sql;
        sql = 'SELECT ' + (this.schema.cred.join(',')) + ' FROM ' + this.table + ' WHERE ' + this.cred_col + '= ? and di= 0';
        return this.core.sqlQuery(ctx, sql, [cred_name]);
      }).then(function(db_rows) {
        return db_rows;
      });
