  ...
  SqlUser = (function() {
    ...
    SqlUser.prototype.create = function(conn, new_values) {
      ...
      for (nm in new_values) {
        ...
        if (__indexOf.call(this.schema.create, nm) < 0) {
          throw new E.DbError('Invalid User Insert Column', {
            col: nm,
            value: val
          });
        }
      ...
      return Q.resolve().then(function() {
        ...
        if (db_result.affectedRows !== 1) {
          throw new E.DbError('Player Insert Failed');
        }
        return db_result;
      });
    };
    ...
    SqlUser.prototype.update = function(conn, id, new_values) {
      ...
      for (nm in new_values) {
        val = new_values[nm];
        if (__indexOf.call(this.schema.create, nm) < 0) {
          throw new E.DbError('Invalid Player Update Column', {
            col: nm,
            value: val
          });
        }
      ...
      return Q.resolve().then(function() {
        ...
        _this.log.debug(f, db_result);
        if (db_result.affectedRows !== 1) {
          throw new E.DbError('Player Update Failed');
        }
        return db_result;
      });
    };
