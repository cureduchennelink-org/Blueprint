...
(function() {
  ...
  SqlUser = (function() {
    ...
    SqlUser.prototype.get_by_ident_id = function(ctx, ident_id) {
      ...
      sql = 'SELECT ' + (this.schema.get_by_ident_id.join(',')) + ' FROM ' + this.ident_tbl + ' i LEFT OUTER JOIN ' + this.table + ' e' + ' ON i.id= e.ident_id WHERE i.id= ? AND i.di= 0 AND (e.di= 0 OR e.id IS NULL)';
