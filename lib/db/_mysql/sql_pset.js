...
class SqlPSet {
	...
	get_by_name(ctx, name){
		...
		.then(() => {
			...
			const sql= `SELECT * FROM ${this.table} WHERE name= ? AND di= 0`;
	...
	read_or_insert(ctx, name){
		...
		.then(() => {
			...
			if (db_rows !== false) {
				if (db_rows.length !== 1) { throw new E.DbError('DB:PUSHSET:REREAD'); }
				existing_pset= db_rows[0];
			}
...
class SqlPSetItem {
	...
	lock(ctx, id){
		...
		.then(() => {
			...
			const sql= `SELECT id FROM ${this.table}` +
	...
	get_psid_xref(ctx, pset_id, xref){
		...
		.then(() => {
			...
			const sql= `SELECT ${this.schema.id_xref.join(',')} FROM ${this.table}` +
	...
	get_by_psid(ctx, pset_id){
		...
		.then(() => {
			...
			const sql= `SELECT ${this.schema.get_psid.join(',')} FROM ${this.table}` +
...
class SqlPSetItemChange {
	...
	GetMostRecentForItem(ctx, pset_id, pset_item_id){
		...
		.then(()=> {
			...
			const sql= `SELECT ${this.schema.recent.join(',')} FROM ${this.table}`+
	...
	GetMostRecentChanges(ctx, limit){
		...
		.then(()=> {
			...
			const sql= `SELECT ${this.schema.next.join(',')} FROM `+
				' (SELECT * FROM '+ this.table+ ' WHERE di= 0'+
	...
	GetNext(ctx, from, limit){
		...
		.then(()=> {
			...
			const sql= `SELECT ${this.schema.next.join(',')} FROM ${this.table}`+
