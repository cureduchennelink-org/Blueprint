...
(function() {
  ...
  Db = (function() {
    ...
    function Db(kit) {
      var PostgreSqlCore, SqlCore, config, i, j, len, len1, log, mod, modPath, model, nm, ref, ref1, ref2;
      log = kit.services.logger.log;
      config = kit.services.config;
      this.config_mongo = false;
      if (config.db.mysql.enable) {
        log.info('Initializing MySql...');
        SqlCore = require(config.db.mysql.type === 'pg' ? './_postgresql/psql_core' : './_mysql/sql_core').SqlCore;
        this.mysql = {
      ...
      if (config.db.psql.enable) {
        ...
        PostgreSqlCore = require('./_postgresql/psql_core').PostgreSqlCore;
        this.psql = {
          core: new SqlCore(kit, config.db.mysql.pool)
        };
