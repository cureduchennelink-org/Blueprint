...
class SqlCore {
  ...
  constructor(kit, pool_opts) {
    ...
    this.acquire= Promise.promisify(this._acquire, {context: this});
    ...
    this.sqlQuery= (ctx, sql, args)=> {
      ...
      if (args && this.is_db_log_on) {
        ctx.log.debug('DB:SqlCore:args:', args);
      }
      if (ctx.conn === null) {
        throw new this.E.DbError('DB:SQL:BAD_CONN');
      }
    ...
    if (schema.UpdateByKey) {
      sql_mod.UpdateByKey= (ctx, key, ids, new_values)=> {
        ...
        if (!schema.UpdateByKey[key]) {
          throw new this.E.DbError(`DB:CORE:SCHEMA_UNDEFINED:UpdateByKey_${key}`);
        }
        ...
        return Promise.resolve().bind(this)
            .then(function() {
              ...
              for (nm in new_values) {
                val = new_values[nm]; cols.push(nm + '= ?'); arg.push(val);
              }
              arg.push(ids);
              const sql= `\
      ...
      const create= (ctx, new_values, re_read)=> {
        ...
        return Promise.resolve().bind(this)
            .then(function() {
              ...
              for (nm in new_values) {
                val = new_values[nm]; cols.push(nm); qs.push('?'); arg.push(val);
              }
              const sql= `\
...
`;
    ...
    if (schema.update_by_id || schema.UpdateById) {
      const update_by_id= (ctx, id, new_values, re_read)=> {
        ...
        if (this.is_db_log_on) {
          ctx.log.debug(f, {id, new_values, re_read});
        }
