...
class SqlRunQueue {
	constructor(core, kit){
		...
		this.db.method_factory(this, 'SqlRunQueue'); // TODO NEEDED?
	}

	GetPendingCnts(ctx){
		const f= "SqlRunQueue:GetPendingCnts:";
...
`;
	...
	GetNextJobs(ctx, max_rows, max_retries){
		...
		const f= "SqlRunQueue:GetNextJobs:";
...
`;
	...
	MarkJobPending(ctx, id, other_values, reread){ // Fails if other_values has nothing
		...
		const f= "SqlRunQueue:MarkJobPending:";
...
`;
	...
	RemoveByIds(ctx, ids){
		const f= "SqlRunQueue:RemoveByIds:";
...
`;
	...
	RemoveByUniqueIds(ctx, uniqueIds){
		const f= "SqlRunQueue:RemoveByUniqueIds:";
...
`;
	...
	GetDelayedByTopic(ctx) {
		const f= "SqlRunQueue:GetDelayedByTopic:";
...
`;
	...
	GetRetriesByTopic(ctx) {
		const f= "SqlRunQueue:GetRetriesByTopic:";
...
`;
	...
	GetFailuresByTopic(ctx) {
		const f= "SqlRunQueue:GetFailuresByTopic:";
