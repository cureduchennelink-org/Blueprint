...
class PSqlRunQueue {
	...
	GetNextJobs(ctx, max_rows, max_retries = 8) {
		...
		// CRB: 05/21/20
		// This was necessary because I was incurring a lot of issues with comparing dates.
		// When I didn't write now like this, we would pick up jobs instantly. Keep this in mind 
		// if you are to edit this query.
		...
		const sql= `
			...
			LIMIT ?
		`;
		return this.db.sqlQuery(ctx, sql, [ now, max_retries, max_rows ]);
	}

	async MarkJobPending(ctx, id, other_values, reread){ // Fails if other_values has nothing
		...
		const updatedRows = await this.db.sqlQuery(ctx, sql, args)
		if (!reread) return updatedRows;
		if (updatedRows.affectedRows !== 1) return [];

		return this.db.sqlQuery(ctx, `SELECT * FROM ${this.table} WHERE ID= ?`, [id])
	}

	async RemoveByIds(ctx, ids){
...
`;
	...
	ReplaceJob(ctx, id, new_values, reread){
		let nm, val;
		if (reread == null) { reread = false; }
		const f= "PSqlRunQueue:ReplaceJob:";
		const e= f;
...
`;
	...
	GetDelayedByTopic(ctx) {
		...
		// TODO WAS TIMESTAMPDIFF(SECOND,MIN(run_at), NOW())
		const sql= `\
SELECT topic, EXTRACT(EPOCH FROM current_timestamp- MIN( run_at)) as delay
FROM ${this.table}
WHERE run_at < current_timestamp and di = 0 and in_process = 0
GROUP BY topic\
`;
		return Promise.resolve().bind(this)
		.then(function() {
