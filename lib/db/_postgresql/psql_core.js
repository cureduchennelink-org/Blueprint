...
class PostgreSqlCore extends CommonCore {
	...
	async sqlQuery(ctx, sql, args = []) {
		...
		this._log(ctx, f+ 'PSQL', statement);
		this._log(ctx, f+ 'ARGS', args);
		const start_time= Date.now();
		const result = await ctx.conn.query(statement, args);

		let log_result= result;
		if (result.rows.length> 3) {
			log_result= _.clone( result); // TODO CONFIRM WE DON'T NEED _.cloneDeep
			log_result.rows= result.rows.slice( 0, 2);
		}
