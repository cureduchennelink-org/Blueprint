...
class PostgreSqlCore extends CommonCore {
	...
	async sqlQuery(ctx, sql, args = [], pure_psql) {
		...
		const result = await ctx.conn.query(statement, args).catch( e=> Promise.resolve().bind(this).delay(10).then( ()=>{ throw e})); // Delay on errors to let conn become queryable

		// Note: Returning early here, won't allow us to track time_ms on these calls. Could have been important in some cases.
		if (result.command=== 'SET' || result.command=== 'START' || result.command=== 'COMMIT' || result.command=== 'ROLLBACK' || result.command=== 'SAVEPOINT'){ return result; }
