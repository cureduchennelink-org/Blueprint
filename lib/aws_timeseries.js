...
class AWS_Timeseries {
    static deps() {
		return {services: ['logger','template','config'], config: 'aws_timeseries[accessKeyId,secretAccessKey,region,databaseName,tableName,dimensions]'};
	}
    ...
    constructor(kit) {
        ...
        AWS.config.update({ accessKeyId: this.config.accessKeyId,
            secretAccessKey: this.config.secretAccessKey, region: this.config.region });
        writeClient.config.region = this.config.region
    ...
    async  _writeRecords(databaseName, tableName, data ) {
        console.log("Writing records");
        // time within data must be a Unix time in milliseconds
        // dimensions is an array of object with Name and Value to add to the MeasurementName and MeasurementValue
        ...
        //     'Time': currentTime.toString()
        // };
        // sample records, this is the main aspect of the write to the timeseries database
        // const records = [{
