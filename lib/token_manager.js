...
(function() {
  ...
  TokenMgr = (function() {
    function TokenMgr(kit) {
      ...
      this.log.info('Initializing Token Manager...');
    ...
    TokenMgr.prototype.decodeAndValidate = function(rawToken, key) {
      ...
      if (parts.length !== 2) {
        throw new E.OAuthError(401, 'invalid_token');
      }
      hmac = crypto.createHmac('sha256', key);
      hmac.update(parts[0]);
      sig = urlSafeBase64EncodeFix(hmac.digest('base64'));
      if (sig !== parts[1]) {
        throw new E.OAuthError(401, 'invalid_token', 'Bad Signature');
      }
      token = JSON.parse((new Buffer(urlSafeBase64DecodeFix(parts[0]), 'base64')).toString('utf8'));
      this.log.debug('tkmgr:', token);
      this.log.debug('moment:', moment().unix());
      this.log.debug('expired: ', token.exp < moment().unix());
