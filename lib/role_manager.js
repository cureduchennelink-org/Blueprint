...
class RoleManager {
	...
	SetCtx(ctx, roleTk, resource_overide){
		...
		if (!ctx.role_def[ctx.resource]) { throw new E.ServerError('ROLEMGR:RESOURCE_NOT_IN_ROLEDEF', ctx.resource); }
	...
	CheckSift(ctx, resource){
		...
		if (!ctx.role_def[resource]) { throw new E.AccessDenied('ROLE:SIFT:RESOURCE_NOT_IN_ROLEDEF', resource); }
		if (!this.sift_def[resource]) { throw new E.AccessDenied('ROLE:SIFT:RESOURCE_NOT_IN_SIFTDEF', resource); }
	...
	CheckTalent(roleTk, wanted_talent){
		const f= 'RoleManager:CheckTalent:';
		this.log.debug(f, {roleTk, wanted_talent});
		if (!Array.from((this.role_def[roleTk] != null ? this.role_def[roleTk].talent : undefined) != null ? (this.role_def[roleTk] != null ? this.role_def[roleTk].talent : undefined) : []).includes(wanted_talent)) { throw new E.AccessDenied(f+'TALENT_LIST'); }
	}

	GetTalentedRoles(talent){
