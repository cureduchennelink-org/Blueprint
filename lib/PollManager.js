...
class PollManager {
	...
	S_CheckOnePollerForItems(poller_for_h, items) {
		...
		items.forEach(item => {
			...
			// JCS Do this at the end, because local postgres gives out-of-order recs: p.count = item.id; // Update the client's count
	...
	S_FinishPoller(poller) {
		...
		for (let nm in listen) {
			...
			// Update the 'count' (max id seen) for this 'h'
			msgs.forEach(({ payload: item }) => {
