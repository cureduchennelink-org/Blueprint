...
class PollManager {
	...
	PollerClosed( poller){ return this.S_CleanupPoller( poller); } // Called when a req connection is closed

	// Called by LongPollRoute to add a new long-poll request
	AddPoller( state, listen, timeout_ms, callback) {
		const f= 'PollManager:AddPoller:';

		const poller={ listen, state, callback, by_h: {}};
	...
	S_CleanupPoller( poller){
		...
		for (let h in poller.by_h) {
			...
			if( this.poller_by_h.size=== 0) delete this.poller_by_h[ h];
		}
		return poller;
	}
