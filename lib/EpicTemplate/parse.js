...
(function() {
  ...
  FindVars = function(text) {
    var args, custom_hash_part, hash_part, i, last, parts, ref;
  ...
  ParseFile = function(file_stats, file_contents) {
    var a, attr, attr_split, clean, empty, finish, i, j, oi, parts, ref, t, tag_wait;
    ...
    while (i < parts.length - 1) {
      ...
      if (parts[i + 1] === '/') {
        ...
        if (parts[i + 3].length > 0) {
          ...
          for (a = j = 0, ref = attr_split.length; j < ref; a = j += 2) {
    ...
    if (tag_wait.length) {
      throw "[" + file_stats + "] Missing closing epic tags" + (((function() {
        var k, len, results;
        results = [];
        for (k = 0, len = tag_wait.length; k < len; k++) {
          t = tag_wait[k];
          results.push(parts[t + 2]);
        }
        return results;
      })()).join(', '));
    }
