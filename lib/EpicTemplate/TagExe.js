...
if window is undefined
	$= extend: (bool,obj,source)-> (require 'lodash').extend obj, source
	window= null
...
class TagExe
	...
	formatFromSpec: (val, spec, custom_spec) ->
		...
		switch spec
			when ''
				@Epic.log2 f, 'typeof', typeof window.EpicMvc.custom_filter
				new_stuff= window.EpicMvc.custom_filter? val, custom_spec
				@Epic.log2 f, 'custom_filter result', new_stuff
				new_stuff
...
if window? then window.EpicMvc.Model.TagExe$Base= TagExe
else module.exports= (w)-> w.EpicMvc.Model.TagExe$Base= TagExe; window= w
