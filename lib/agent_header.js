...
class AgentHeader {
	static initClass() {
		this.deps = {services: ['logger' ,'config' ], mysql: [ 'agent_header'], config: 'agent_header[count_max]'};
	}
	constructor(kit) {
		...
		this.count_max= (config != null ? config.count_max : undefined) != null ? (config != null ? config.count_max : undefined) : 200;
	...
	xlate(agent_header_string){
		...
		if (id != null) { return id; } // Nice, it was cached
		...
		.then(function() {
			...
			for (let nm in object) {
				const rec = object[nm];
				for (let sub_nm in rec) { const val = rec[sub_nm]; more[ `${nm}_${sub_nm}`]= val; }
			}
			return this.sdb.agent_header.InsertIfNew(this.ctx, agent_header_string, more);}).then(function(db_results){
			// TODO IS THIS ALLOWED TO FAIL?
			id= db_results.insertId;
			return this._add(agent_header_string, id);}).then(() => id);
	}
