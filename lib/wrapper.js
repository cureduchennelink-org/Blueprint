...
(function() {
  var Promise, Wrapper, _, request_count, request_count_high,
  ...
  Wrapper = (function() {
    ...
    function Wrapper(kit) {
      ...
      this.lamd = (ref = kit.services.lamd) != null ? ref : stubb_lamd;
    ...
    Wrapper.prototype.auth = function(req, res, next, endpoint) {
      var ctx, f, p, pre_loaded, ref, ref1, ref2, ref3, result, route_logic, supported_grant_type;
      f = "Wrapper:auth";
      ...
      pre_loaded = {};
      result = false;
      supported_grant_type = (ref3 = p.grant_type) === 'password' || ref3 === 'refresh_token' || ref3 === 'client_credentials' ? true : false;
      return Promise.resolve().bind(this).then(function() {
        ...
        if (ctx.conn !== null && ctx.conn._protocol._fatalError !== null) {
          throw new this.E.ServerError(f + 'BadHandle:' + JSON.stringify(this._exposeErrorProperties(ctx.conn._protocol._fatalError)));
        }
        ...
        if ((ref4 = result.send) != null) {
          ref4.req_uuid = ctx.lamd.req_uuid;
        }
        ...
        if (endpoint.lamd !== false) {
          this.lamd.write_deep(ctx);
        }
        this.end_connection_limit();
        return next();
      })["catch"](function(err) {
        var e, end, ref4, ref5;
        if ((ref4 = err.statusCode) !== 400 && ref4 !== 401 && ref4 !== 403) {
        ...
        e = err.body ? err : new this.E.ServerError(err.name, err.message);
        e.body.req_uuid = ctx.lamd.req_uuid;
        ctx.lamd.statusCode = (ref5 = err.statusCode) != null ? ref5 : 5550;
        end = new Date().getTime();
        ctx.lamd.duration = end - ctx.lamd.start;
        ...
        this.lamd.write_deep(ctx);
        this.end_connection_limit();
        e.toJSON = (function(_this) {
          return function() {
            return _this._exposeErrorProperties(e.body);
          };
        })(this);
        return next(e);
      });
    ...
    Wrapper.prototype["default"] = function(req, res, next, endpoint) {
      var ctx, f, p, pre_loaded, ref, ref1, ref2, ref3, ref4, result, route_logic;
      ...
      if (endpoint.auth_required || endpoint.permit) {
        if (((ref3 = this.config.perf) != null ? ref3.test_user_override : void 0) === true && (p.mock_id != null)) {
          req.auth.authId = Number(p.mock_id);
          req.auth.role = (ref4 = p.mock_role) != null ? ref4 : 'mock_role';
        } else {
          if (!req.auth.authorize()) {
            return next();
          }
      ...
      return Promise.resolve().bind(this).then(function() {
        var aRole, accessDeniedError, role, roles;
        accessDeniedError = (function(_this) {
          return function(message) {
            throw new _this.E.AccessDenied(f + " " + message);
          };
        })(this);
        if (endpoint.domain) {
        ...
        if (endpoint.roles) {
          ...
          if (!roles || roles.length === 0 || !Array.isArray(roles)) {
            accessDeniedError('MISSING ROLE');
          }
          role = (function() {
            var i, len, results;
            results = [];
            for (i = 0, len = roles.length; i < len; i++) {
              aRole = roles[i];
              if (indexOf.call(endpoint.roles, aRole) >= 0) {
                results.push(aRole);
              }
            }
            return results;
          ...
          if (role.length === 0) {
            accessDeniedError('INVALID ROLE');
          }
        }
        this.start_connection_limit();
        ctx.lamd.request_count = request_count;
        return ctx.lamd.request_count_high = request_count_high;
      }).then(function() {
        ...
        var fn, func, nm, q_result, ref5;
        q_result = Promise.resolve().bind(this);
        ref5 = endpoint.pre_load;
        fn = (function(_this) {
        ...
        if ((ref5 = result.send) != null) {
          ref5.req_uuid = ctx.lamd.req_uuid;
        }
        ...
        var e, end, ref5, ref6;
        delete ctx.pool;
        if ((ref5 = err.statusCode) !== 400 && ref5 !== 403) {
          req.log.error(f, '.catch', err, err.stack);
        } else {
        ...
        e = err.body ? err : new this.E.ServerError(err.name, err.message);
        e.body.req_uuid = ctx.lamd.req_uuid;
        ctx.lamd.statusCode = (ref6 = err.statusCode) != null ? ref6 : 5550;
        end = new Date().getTime();
        ctx.lamd.duration = end - ctx.lamd.start;
        ctx.lamd.err = this._exposeErrorProperties(err);
        this.lamd.write(ctx.lamd);
