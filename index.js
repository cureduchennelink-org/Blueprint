...
exports.start= function(include_server, services_enabled, routes_enabled, mysql_enabled, mysql_mods_enabled, mongo_enabled, more_config, more_kit) {
  ...
  for (nm in kit.services) {
    service = kit.services[nm];
    if (typeof service.serverStart === 'function') {
      (((service) => q_result= q_result.then(() => service.serverStart(kit))))(service);
    }
  }
  ...
  if (server) {
    q_result= q_result.then(function() {
      ...
      if (config.api != null ? config.api.static_file_server : undefined) {
...
var update_deps= function(kit, services_enabled, routes_enabled, mysql_mods_enabled) {
  let dep; let deps; let mod; let module; let servicePath;
  let start_length;
  let nm; let service; let child;
  ...
  for (nm of Array.from(routes_enabled)) {
    ...
    for (const snm of Array.from(deps)) {
      service_to_deps[snm]= false;
    }
  } // Add all services that any route depends on

  for (nm of Array.from(special)) {
    service_to_deps[nm]= [];
  } // These are base services with no deps that are always 'provided'
  ...
  for (nm in service_to_deps) {
    ...
    if (s2child[nm] == null) {
      s2child[nm] = [];
    } // Make sure I exist here, even if no deps
    ...
    for (service in s2child) {
      ...
      if (children.length === 0) {
        ...
        const min= _.min(((() => {
          const result3 = [];
          for (child of Array.from(children)) {
            result3.push(all_services.indexOf(child));
          }
          return result3;
        })()));
        if (min < 0) {
