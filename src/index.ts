...
const blueprintIQ = (configOverrides = {}) => {
  const config = Config(configOverrides);
  let kit = new Kit(config.log);

  let logger = new Logger();

  kit.addService("logger", logger);
  // kit.addService("error", Error);
  kit = merge(kit); //, more_kit);

  const { services } = kit;
  // const { logger, restify_logger } = services;
  const server = new Server(config, logger, restify_logger);
  server.create();
  kit.addService("server", server);

  // [servicesEnabled, mysqlModsEnabled] = updateDependencies(
  //   kit,
  //   servicesEnabled,
  //   routesEnabled,
  //   mysqlModsEnabled
...
//       //throw new Error f+ "Class (#{mod.class}) not found in file (#{servicePath})" unless mod.class of module
//       const dependencies = kit.getServiceDependenciesNeeded(
//         name,
//         module[mod.class]
//       );
...
//       if (present.hasOwnProperty(service)) continue;
