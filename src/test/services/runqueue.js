...
(function() {
  ...
  mongodb_uri =
    (ref = process.env.npm_config_mongodb_uri) != null
      ? ref
      : "mongodb://localhost";

  mongodb_name =
    (ref1 = process.env.npm_config_mongodb_name) != null ? ref1 : "test";
  ...
  showme = function(obj) {
    ...
    return console.log("SHOWME:" + typeof obj, {
      ...
      name:
        (ref2 = (ref3 = obj.constructor) != null ? ref3.name : void 0) != null
          ? ref2
          : "no-name"
    });
  };
  ...
  clean_poll_result = function(result, debug) {
    ...
    for (i = 0, len = result.length; i < len; i++) {
      ...
      if (
        it_is.array(
          (ary =
            line.next_jobs || line.process_result || line.MarkJobPending_result)
        )
      ) {
        if (debug) {
          _log(f + "DB", {
          ...
          (function() {
            ...
            for (j = 0, len1 = ref2.length; j < len1; j++) {
              ...
              if (debug) {
                _log(f + "OBJ?", {
                  o: line[obj_key],
                  t: typeof ((ref3 = line[obj_key]) != null
                    ? ref3.affectedRows
                    : void 0)
                });
              }
              if (
                it_is.object(line[obj_key]) &&
                it_is.number(line[obj_key].affectedRows)
              ) {
      ...
      return it("_Poll topic_fails", function() {
        return Promise.resolve()
          ...
          .then(function(result) {
            ...
            result[4].should.have.a
              .property("topic_method_error")
              .that.is.an.instanceOf(Error);
            error = result[4].topic_method_error;
            return jobs_to_expect_at_the_end.push(
              _.merge({}, base_job_result["topic_fail"], {
        ...
        return Promise.delay(1000)
          ...
          .then(function(result) {
            ...
            job_active = _.merge(_.clone(job), {
              in_process: 1,
              fail_at: moment()
                .add(fail_default[0], fail_default[1])
                .format()
            });
            ...
            jobs_to_expect_at_the_end.push(
              _.merge(_.clone(job_active), {
                di: 1
              })
            );
            return jobs_to_expect_at_the_end.push(
              _.merge(_.clone(job1_active), {
                di: 1
              })
            );
          })
        ...
        return Promise.resolve()
          ...
          .then(function(job) {
            ...
            fail_promises = (function() {
              ...
              results = [];
              for (
                i = 1, ref2 = expected_retries;
                1 <= ref2 ? i <= ref2 : i >= ref2;
                1 <= ref2 ? i++ : i--
              ) {
                results.push(
                  runqueue_service.sdb.runqueue.Fail(ctx, job[0].id, values)
                );
              }
              return results;
            })();
          ...
          .then(function(result) {
            return result.details.retries.should.deep.equal(
              expected.details.retries
            );
          });
      });
        ...
        return Promise.resolve()
          ...
          .then(function(job) {
            ...
            fail_at = moment()
              .add(0, "s")
              .format();
            return runqueue_service.sdb.runqueue.MarkJobPending(
              ctx,
              job[0].id,
              {
                fail_at: fail_at
          ...
          .then(function(result) {
            return result.details.failures.should.deep.equal(
              expected.details.failures
            );
          });
      });
