...
(function() {
  ...
  Db = (function() {
    ...
    Db.prototype.open = function() {
      ...
      return Promise.resolve()
        ...
        .then(function(client) {
          ...
          this.log(f + "mdb", _.pick(mdb, ["databaseName", "options"]));
          if (mdb == null) {
            throw new Error(f + "MongoDB connection is empty");
          }
          this.db = mdb;
          this.runqueue = mdb.collection("runqueue");
          this.log(f + "collection-runqueue", this.runqueue);
          this.log(f + "collection-runqueue-keys", {
            runqueue: this.runqueue,
            keys: Object.keys(this.runqueue)
          });
          return this.log(f + "collection-runqueue-funcs", {
            functions: function() {
              var ref, results;
              ref = this.runqueue;
              results = [];
    ...
    Db.prototype.xSqlQuery = function(sql, args) {
      ...
      return p_query(sql, args)
        .bind(this)
        .then(function(just_rows) {
          console.log(
            "----RESULT-> ",
            "affectedRows" in just_rows ? JSON.stringify(just_rows) : just_rows
          );
          return just_rows;
        });
    };
    ...
    Db.prototype.xGetByKey = function(table, key, vals) {
      ...
      return Promise.resolve()
        ...
        .then(function() {
          var args, sql;
          args =
            vals_type === "number" || vals_type === "string"
              ? [[vals]]
              : [vals];
          sql =
            "SELECT * FROM " + table + " WHERE di= 0 AND " + key + " IN (?)";
          return this.SqlQuery(sql, args);
        })
        .then(function(db_rows) {
          return db_rows;
