...
class Registration {
	static initClass() {
		this.deps= {services:[ 'error', 'config', 'ses', 'auth', 'tripMgr', 'template', ], mysql:[ 'auth', 'user', ], config: 'ses.options,api.ident_id'};
	}
	constructor(kit){
	...
	_signup(ctx, pre_loaded){
		...
		if (!p.fnm) { throw new this.E.MissingArg('fnm'); }
		if (!p.lnm) { throw new this.E.MissingArg('lnm'); }
		if (!p.role) { throw new this.E.MissingArg('role'); }
		...
		.then(function() {
			...
			if (db_rows.length !== 0) { throw new this.E.AccessDenied('REGISTER:SIGNUP:EMAIL_EXISTS'); }

			// Create Trip and store email, fnm, lnm in json info. Never Expires (null).
			const expires = 3; //expires in three days
			const expireDate = moment().add(expires, 'days').format();
			return this.tripMgr.planTrip(ctx, this.config.api.ident_id, { eml: p.eml, fnm: p.fnm, lnm: p.lnm, role: p.role }, expireDate, 'signup');}).then(function(new_trip){
			ctx.log.debug(f, 'got signup round trip:', new_trip);
			const trip= new_trip;

			// Send Signup email
			recipient= {eml: p.eml, fnm: p.fnm, lnm: p.lnm};
			return this.ses.send('verify_signup', this.make_tbl(recipient, trip.token, this.config.ses.options));}).then(function(){
			success= true;
	...
	_read_signup(ctx, pre_loaded){
		...
		.then(function() {
			...
			if (bad_token) { throw new this.E.AccessDenied('REGISTER:READ_SIGNUP:BAD_TOKEN'); }
			trip.json= JSON.parse(trip.json);

			// Verify email doesn't already exist
			return this.sdb.auth.GetByCredName(ctx, trip.json.eml);}).then(function(db_rows){
			ctx.log.debug('got ident with eml:', db_rows);
			if (db_rows.length !== 0) { throw new this.E.AccessDenied('REGISTER:READ_SIGNUP:EMAIL_EXISTS'); }
			success= true;
	...
	_register_signup(ctx, pre_loaded){
		...
		.then(function() {
			...
			trip= trip_info;
			const bad_token= (trip_info.status === 'unknown') || (trip_info.status !== 'valid');
			if (bad_token) { throw new this.E.AccessDenied('REGISTER:REGISTER_SIGNUP:BAD_TOKEN'); }
			trip.json= JSON.parse(trip.json);
			eml_change= eml !== trip.json.eml;

			// Verify email doesn't already exist
			return this.sdb.auth.GetByCredName(ctx, eml);}).then(function(db_rows){
			ctx.log.debug(f, 'got ident with eml:', db_rows);
			if (db_rows.length !== 0) { throw new this.E.AccessDenied('REGISTER:REGISTER_SIGNUP:EMAIL_EXISTS'); }
			success= true;
			...
			return this.sdb.auth.Create(ctx, new_ident);}).then(function(db_result){
			if (db_result.affectedRows !== 1) { throw new this.E.DbError('REGISTER:REGISTER_SIGNUP:CREATE_IDENT'); }
			...
			return this.sdb.user.Create(ctx, new_profile);}).then(function(db_result){
			if (db_result.affectedRows !== 1) { throw new this.E.DbError('REGISTER:REGISTER_SIGNUP:CREATE_PROFILE'); }

			// Return the Trip to the Trip Manager
			return this.tripMgr.returnFromTrip(ctx, trip.id, new_ident_id);}).then(function(){

			// Send Signup Complete Email
			if (eml_change) { return false; }
			const recipient= {email: p.eml, fnm: p.fnm, lnm: p.lnm};
			return this.ses.send('signup_complete', this.make_tbl(recipient));}).then(function(){

			// Create Trip and store email in json info
			if (!eml_change) { return false; }
			return this.tripMgr.planTrip(ctx, new_ident_id, { eml }, null, 'update_email');}).then(function(new_trip){
			...
			return this.ses.send('verify_email_change', this.make_tbl(recipient, change_trip.token));}).then(function(){
			success= true;
