...
describe('Sql Core', function(){
	const core= new SqlCore(config.db.mysql.pool, {debug: console.log});
	let conn= false;
	...
	it('should acquire a connection from the pool', done=>
		core.acquire(function(err, connection){
			if (err) { done(err); }
			conn= connection;
	...
	it('should start a transaction against a context', function(){
		...
		.then(function(c){
			...
			return core.StartTransaction(ctx);}).then(()=> core.StartTransaction(ctx)).then(()=> null).fail(function(err){
