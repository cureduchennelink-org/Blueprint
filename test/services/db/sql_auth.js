...
const {SqlCore}= 	require('../../../lib/db/mysql/sql_core');
...
}).then((db_rows) => db_rows[0].should.deep.equal({id: ident.id, eml: ident.eml, role: 'r', tenant: 't'}))
...
}).then((db_rows) => db_rows[0].should.deep.equal(ident)).catch(function(err) {
          _log.debug({err});
          if (ctx.conn !== null) {
core.release(ctx.conn);
}
...
}).then((db_rows) => db_rows[0].should.deep.equal({id: ident.id, pwd: ident.pwd, role: 'r', tenant: 't'}))
