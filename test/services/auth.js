...
const encryptedPassword =
  "xfGuZKjVkoNgQyXxYT8+Hg==.f+uw2I+dqzfOE4O82Znikrbdb0lOONBxl/xcWGsQtFI=";
...
let req = new Mock.RestifyRequest({
  url: "localhost/api",
  params: { p1: "p1", p2: "p2" }
});
...
describe("Auth Service", function() {
  const { auth } = kit.services; // shortcut
  ...
  it("should Encrypt passwords using variable salt", function() {
    ...
    return auth
      ...
      .then(function(encryption) {
        [salt1, hash1] = encryption;
        console.log({ salt1 });

        return auth.EncryptPassword("password");
      })
      .then(function(encryption) {
        [salt2, hash2] = encryption;
  ...
  it("should tell if a password matches an encrytped hash", function() {
    auth.ComparePassword("password", encryptedPassword).should.eventually.be
      .true;
    auth.ComparePassword("password1", encryptedPassword).should.eventually.be
      .false;
    return auth.ComparePassword("password", encryptedPassword + "BROKEN").should
      .eventually.be.false;
  });

  it("should validate a username and password combination", () =>
    auth
      .ValidateCredentials({}, "test@email.com", "password")
  ...
  return describe("server_use", function() {
    ...
    const good_token = kit.services.tokenMgr.encode(
      user_data,
      future,
      config.auth.key
    );
    const expired_token = kit.services.tokenMgr.encode(
      { iid: 42 },
      moment(),
      config.auth.key
    );
    ...
    return describe("authorize()", function() {
      ...
      it("should be false for an un-authorized request", function() {
        let skip;
        req = new Mock.RestifyRequest({
          params: { auth_token: `${expired_token}` }
        });
        auth.server_use(req, {}, function() {});
